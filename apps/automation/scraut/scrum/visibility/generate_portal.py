"""
scrum/visibility/generate_portal.py
Render the static HTML stakeholder portal from sprint data.
Writes bot-generated files to apps/portal/ (atomic_write is safe here).
"""
import argparse
import json
import logging
...
from scraut.platform.utils.config import (
    ...
    load_config,
    get_sprint_folder,
    get_sprint_output_folder,
    get_scraut_root,
)
from scraut.platform.utils.date_utils import get_sprint_dates, days_until_sprint_end
...
def _load_sprint_meta(sprint_num: int) -> str:
    return read_file(get_sprint_folder(sprint_num) / "meta.md")


def _load_standup_summary(sprint_num: int, today: str) -> str:
    return read_file(
        get_sprint_output_folder(sprint_num) / "standup" / "summary" / f"{today}.md"
    )


def _load_velocity_insight() -> Optional[float]:
    content = read_file(get_scraut_root() / "insights" / "velocity-trends.md")
    if not content:
        return None
...
def generate_form_html(config: dict) -> None:
    """
    Generate apps/portal/form.html with team members populated from scraut.yml.
    This replaces the static CONFIG.teamMembers = [] with actual data.
    """
    portal_dir = get_portal_root()
    form_template = read_file(portal_dir / "form.html")
    ...
    atomic_write(portal_dir / "form.html", updated)
...
def generate_portal(config: dict, dry_run: bool = False) -> None:
    ...
    if dry_run:
        logger.info("[DRY-RUN] Would write apps/portal/index.html")
        logger.info("[DRY-RUN] Would write apps/portal/style.css")
        logger.info("[DRY-RUN] Would write apps/portal/data.json")
