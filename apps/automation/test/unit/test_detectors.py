...
def test_velocity_drop_no_drop(scraut_repo, config):
    ...
    with patch("scraut.scrum.suggestions.detectors.calculate_rolling_velocity") as mock_roll:
        with patch("scraut.scrum.suggestions.detectors.calculate_sprint_velocity") as mock_vel:
...
def test_capacity_imbalance_balanced_team(scraut_repo, config, mock_repo):
    ...
    for issue in mock_repo._issues.values():
        issue.assignees = []
    with patch("scraut.scrum.suggestions.detectors.get_github_client") as mock_g:
