...
from scripts.llm.prompts import WEEKLY_DIGEST_NARRATIVE
from scripts.notifications.send_email import send_email
from scripts.notifications.slack_post import post_to_slack
from scripts.utils.config import get_current_sprint, get_repo_root, load_config
...
def _load_sprint_review(sprint_num: int) -> str:
    return read_file(get_repo_root() / f"sprint-{sprint_num:02d}" / "review" / "sprint-review.md")


def _load_velocity_insights() -> str:
    return read_file(get_repo_root() / "insights" / "velocity-trends.md")


def _load_blocker_insights() -> str:
    return read_file(get_repo_root() / "insights" / "blocker-patterns.md")
...
def _load_standup_summaries(sprint_num: int, days_back: int = 5) -> list[str]:
    ...
    for i in range(days_back):
        ...
        content = read_file(root / f"sprint-{sprint_num:02d}" / "standup" / "summary" / f"{d}.md")
        if content:
