...
from tests.mocks.llm_mocks import TRIAGE_RESPONSE
...
def test_triage_applies_labels(scraut_repo, config):
    ...
    with patch("scripts.backlog.triage_issue.get_github_client") as mock_g:
        mock_g.return_value.get_repo.return_value = repo
        with patch("scripts.backlog.triage_issue.complete_json", return_value=TRIAGE_RESPONSE):
            with patch("scripts.backlog.triage_issue.calculate_rolling_velocity",
                       return_value={"avg": 26.0, "sprints_sampled": 3, "std_dev": 2.0}):
                from scripts.backlog.triage_issue import triage_issue
